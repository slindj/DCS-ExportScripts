...
function ExportScript.Tools.createUDPSender()
    ...
    if lerror ~= nil then
        ExportScript.Tools.WriteToLog("createUDPSender protect: " ..
            ExportScript.Tools.dump(ln) .. ", " .. ExportScript.Tools.dump(lerror))
        return
    end
...
function ExportScript.Tools.createUDPListner()
    if ExportScript.Config.Listener then
        ...
        if lerror ~= nil then
            ExportScript.Tools.WriteToLog("createUDPListner protect: " ..
                ExportScript.Tools.dump(ln) .. ", " .. ExportScript.Tools.dump(lerror))
            return
        end
...
function ExportScript.Tools.ProcessInput()
    ...
    if ExportScript.Config.Listener then
        ...
        local lUDPListenerReceivefrom = ExportScript.socket.protect(function()
            ...
            ExportScript.UDPListenerValues.Input, ExportScript.UDPListenerValues.from, ExportScript.UDPListenerValues.port =
                ExportScript.socket.try(ExportScript.UDPListener:receivefrom())
        end)

        local ln, lerror = lUDPListenerReceivefrom()
        if lerror ~= nil and lerror ~= "timeout" then
            ExportScript.Tools.WriteToLog("UDPListenerReceivefrom protect: " ..
                ExportScript.Tools.dump(ln) .. ", " .. ExportScript.Tools.dump(lerror))
            ExportScript.UDPListener:close()
            ExportScript.Tools.createUDPListner()
        end

        local lInput, from, port = ExportScript.UDPListenerValues.Input, ExportScript.UDPListenerValues.from,
            ExportScript.UDPListenerValues.port

        if ExportScript.Config.SocketDebug then
            ExportScript.Tools.WriteToLog("lInput: " ..
                ExportScript.Tools.dump(lInput) ..
                ", from: " .. ExportScript.Tools.dump(from) .. ", port: " .. ExportScript.Tools.dump(port))
        end
        if lInput then
            ...
            if (lCommand == "C") then
                ...
                elseif lDeviceID < 1000 then
                    ...
                    if ExportScript.FoundDCSModule and type(lDevice) == "table" then
                        ...
                        if ExportScript.Config.Debug then
                            ExportScript.Tools.WriteToLog("performClickableAction for Device: " ..
                                lCommandArgs[1] .. ", ButtonID: " .. lCommandArgs[2] .. ", Value: " .. lCommandArgs[3])
                        end
                    end
                elseif lDeviceID == 1000 then
                    ...
                    if ExportScript.FoundDCSModule then
                        ...
                        if ExportScript.Config.Debug then
                            ExportScript.Tools.WriteToLog("genericRadio, ButtonID: " ..
                                lCommandArgs[2] .. ", Value: " .. lCommandArgs[3])
                        end
                    end
...
function ExportScript.Tools.ProcessOutput()
    ...
    if type(lDevice) == "table" and ExportScript.FoundDCSModule then
        ...
        if ExportScript.Config.Debug then
            ...
            coStatus = coroutine.resume(ExportScript.coProcessArguments_EveryFrame, lDevice,
        ...
        if ExportScript.Config.IkarusExport then
            if ExportScript.Config.Debug then
                ...
                ExportScript.coProcessIkarusDCSHighImportance = coroutine.create(ExportScript
                    .ProcessIkarusDCSHighImportance)
        ...
        if ExportScript.lastExportTimeHI > ExportScript.Config.ExportLowTickInterval then
            ...
            if ExportScript.Config.IkarusExport then
                if ExportScript.Config.Debug then
                    ...
                    ExportScript.coProcessIkarusDCSLowImportance = coroutine.create(ExportScript
    ...
    elseif ExportScript.FoundFCModule then       -- Assume FC Aircraft
        ...
        if ExportScript.Config.IkarusExport then
            if ExportScript.Config.Debug then
                ...
                ExportScript.coProcessGlassCockpitFCHighImportance = coroutine.create(ExportScript
                    .ProcessIkarusFCHighImportance)
        ...
        if ExportScript.lastExportTimeHI > ExportScript.Config.ExportLowTickInterval then
            if ExportScript.Config.IkarusExport then
                if ExportScript.Config.Debug then
                    ...
                    ExportScript.coProcessIkarusFCLowImportance = coroutine.create(ExportScript
                        .ProcessIkarusFCLowImportance)
...
function ExportScript.Tools.ProcessArguments(device, arguments)
    ...
    for lArgument, lFormat in pairs(arguments) do
        ...
        if ExportScript.Config.Debug then
            lCounter = lCounter + 1
            ExportScript.Tools.WriteToLog(lCounter ..
                ". ID: " .. lArgument .. ", Fromat: " .. lFormat .. ", Value: " .. lArgumentValue)
        end
        if ExportScript.Config.IkarusExport then
...
function ExportScript.Tools.SendDataDAC(id, value)
    for hardware = 1, #ExportScript.Config.DAC, 1 do
        ...
        if ExportScript.Config.DAC[hardware] == nil then
            ExportScript.Tools.WriteToLog("unknown hardware ID '" .. hardware .. "' for value: '" .. id ..
                "=" .. value .. "'")
            return
        end
...
function ExportScript.Tools.FlushData()
    local lFlushData = ExportScript.socket.protect(function()
        if #ExportScript.SendStrings > 0 then
            ...
            local lPacket = lES_SimID ..
                table.concat(ExportScript.SendStrings, ExportScript.Config.IkarusSeparator) .. "\n"
            --ExportScript.socket.try(ExportScript.UDPsender:sendto(lPacket, ExportScript.Config.IkarusHost, ExportScript.Config.IkarusPort))
            local try = ExportScript.socket.newtry(function()
            ...
            if ExportScript.Config.SocketDebug then
                ExportScript.Tools.WriteToLog("FlushData: send to host: " ..
                    ExportScript.Config.IkarusHost ..
                    ", Port: " .. ExportScript.Config.IkarusPort .. ", Data: " .. lPacket)
            end
    ...
    if lerror ~= nil then
        ExportScript.Tools.WriteToLog("FlushData protect: " ..
            ExportScript.Tools.dump(ln) .. ", " .. ExportScript.Tools.dump(lerror))
    end
end
...
function ExportScript.Tools.FlushDataDAC(hardware)
    ...
    local lFlushDataDAC = ExportScript.socket.protect(function()
        if #ExportScript.SendStringsDAC[hardware] > 0 then
            local lPacket = ExportScript.SimID ..
                table.concat(ExportScript.SendStringsDAC[hardware], ExportScript.Config.DAC[hardware].Separator) .. "\n"
            ...
            try(ExportScript.UDPsender:sendto(lPacket, ExportScript.Config.DAC[hardware].Host,
                ExportScript.Config.DAC[hardware].SendPort))

            if ExportScript.Config.SocketDebug then
                ExportScript.Tools.WriteToLog("FlushDataDAC[" ..
                    hardware ..
                    "]: send to host: " ..
                    ExportScript.Config.DAC[hardware].Host ..
                    ", Port: " .. ExportScript.Config.DAC[hardware].SendPort .. ", Data: " .. lPacket)
            end
    ...
    if lerror ~= nil then
        ExportScript.Tools.WriteToLog("FlushDataDAC protect: " ..
            ExportScript.Tools.dump(ln) .. ", " .. ExportScript.Tools.dump(lerror))
    end
end
            ...
            if info.what == "C" then
                ...
                if (string.sub(info.source, 1, 2) == [[./]]) then
                    return string.format('%q',
                            fcnname ..
                            ', defined in (' .. info.linedefined .. '-' .. info.lastlinedefined .. ')' .. info.source) ..
                        '\n'
                else
                    return string.format('%q',
...
function ExportScript.Tools.getListIndicatorValue(IndicatorID)
    ...
    if ExportScript.Config.Debug then
        ExportScript.Tools.WriteToLog('list_indication(' .. IndicatorID .. '): ' ..
            ExportScript.Tools.dump(ListIindicator))
    end
